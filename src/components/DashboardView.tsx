...
const Countdown = () => {
    ...
    if (today.getMonth() === 11 && today.getDate() > 31) nye = new Date(currentYear + 1, 11, 31);
    ...
    <div className="h-full overflow-y-auto space-y-8 animate-in fade-in pb-10 pr-2">
        ...
        <div className="card-panel p-8 rounded-3xl bg-white/90 border-teal-100">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div className="flex-1">
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2">
                        {[15, 30, 45, 60, 9999].map((val) => (
                            <button key={val} onClick={() => setSelectedTime(val)} className={`py-2 px-6 rounded-full text-sm font-bold transition-all whitespace-nowrap transform duration-200 shadow-sm ${selectedTime === val ? 'bg-teal-600 text-white shadow-teal-200 scale-105' : 'bg-white text-stone-600 border border-stone-200 hover:border-teal-300 hover:text-teal-700'}`}>
                                {val === 9999 ? 'All' : `${val}m`}
                            </button>
                        ))}
                    </div>
                ...
                <div className="md:w-auto w-full">
                    <button onClick={() => onSwitchView('execute')} className="w-full md:w-auto px-10 py-4 text-base rounded-full font-bold flex items-center gap-2 justify-center transition-all active:scale-95 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white shadow-lg shadow-emerald-100 hover:shadow-emerald-200">
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-end px-2">
                ...
                <button onClick={() => onSwitchView('inventory')} className="text-xs font-bold text-teal-700 hover:text-teal-900 bg-teal-50 hover:bg-teal-100 px-5 py-2.5 rounded-full transition-colors flex items-center gap-2 shadow-sm border border-teal-100">
                    <Edit className="w-3 h-3" /> Edit Tasks
                </button>
            </div>
            <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-3">
                {zones.length === 0 && <div className="text-sm text-stone-500 italic p-10 text-center bg-white/50 rounded-3xl border border-dashed border-stone-300 col-span-full">No areas defined yet.</div>}
                {zones.map(zone => {
                    ...
                    return (
                        <div key={zone} className="card-panel p-6 rounded-3xl flex flex-col justify-between transition-all group border-l-0 card-hover cursor-pointer relative overflow-hidden" onClick={() => onFilterZone(zone)}>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); onTackleArea(zone); }} className="w-full py-2.5 text-[11px] font-bold text-stone-600 hover:text-teal-700 bg-stone-50 hover:bg-teal-50 rounded-xl flex items-center justify-center gap-2 transition-colors uppercase tracking-widest border border-stone-200">
